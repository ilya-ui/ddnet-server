...
void CControls::OnRender()
{
	...
	// Aimbot logic
	if(g_Config.m_ClAimbot && GameClient()->m_Snap.m_pLocalCharacter && !GameClient()->m_Snap.m_SpecInfo.m_Active)
	{
		bool DoAim = true;
		if(g_Config.m_ClAimbotOnlyHook)
		{
			DoAim = (m_aInputData[g_Config.m_ClDummy].m_Hook != 0);
		}
		...
		if(DoAim)
		{
			...
			int ClosestId = -1;
			float ClosestDist = (float)g_Config.m_ClAimbotRange;
			...
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				...
				float IntraTick = Client()->IntraGameTick(g_Config.m_ClDummy);
				vec2 EnemyPos = vec2(
			...
			if(ClosestId >= 0)
			{
				CNetObj_Character Prev = GameClient()->m_Snap.m_aCharacters[ClosestId].m_Prev;
				CNetObj_Character Cur = GameClient()->m_Snap.m_aCharacters[ClosestId].m_Cur;
				float IntraTick = Client()->IntraGameTick(g_Config.m_ClDummy);
				vec2 EnemyPos = vec2(
					mix((float)Prev.m_X, (float)Cur.m_X, IntraTick),
					mix((float)Prev.m_Y, (float)Cur.m_Y, IntraTick)
	...
	// Draw aimbot range lines
	if(g_Config.m_ClAimbotShowRange && GameClient()->m_Snap.m_pLocalCharacter && !GameClient()->m_Snap.m_SpecInfo.m_Active)
	{
		vec2 LocalPos = GameClient()->m_LocalCharacterPos;
		float Range = (float)g_Config.m_ClAimbotRange;
		...
		for(int i = 0; i < Segments; i++)
		{
			...
			IGraphics::CLineItem Line(
				LocalPos.x + cosf(Angle1) * Range,
				LocalPos.y + sinf(Angle1) * Range,
				LocalPos.x + cosf(Angle2) * Range,
				LocalPos.y + sinf(Angle2) * Range);
